...
{
    m_contacts.append({ "Angel", "Chapel St. 36866 ", "Clearwater" , "0311 1823993" });
...
{
    int row = 0;
    qDebug() << "remove" << m_contacts.at(row).fullName << m_contacts.count();
    while (row < m_contacts.count() && fullName > m_contacts.at(row).fullName){
        ++row;
        qDebug() << "remove2" << m_contacts.at(row).fullName;
    }
    beginInsertRows(QModelIndex(), row, row);
    m_contacts.insert(row, {fullName, address, city, number});
    endInsertRows();
    qDebug() << "remove3 " << row << m_contacts.at(row).fullName << m_contacts.count();
}
